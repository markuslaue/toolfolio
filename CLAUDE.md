...
2. **Vermittler-Prinzip.** Kein Hosting fremder Software, keine Drittzahlungsabwicklung. Leads gehen an den Anbieter (E-Mail/Webhook).
   **Bewusst geaendert (2026-07-14, Markus):** Toolfolio speichert Anfragen jetzt **doch** personenbezogen, weil der **Launch-Service (A-06)** darauf aufbaut. Zwei strikt getrennte Rechtsgrundlagen, nie vermischen:
   - **Anfrage weiterleiten** = Vertragsanbahnung (Art. 6 I b). Keine Einwilligung noetig, Speicherung nur zur Erfuellung und zum Nachweis.
   - **Launch-Service** (spaeterer Kontakt durch andere Anbieter) = **Einwilligung** (Art. 6 I a). **Eigene, leere, freiwillige Checkbox**, Zweck im Klartext, **Double-Opt-in**, Wortlaut + Zeitpunkt protokolliert, jederzeit widerrufbar.
   **Verboten:** Einwilligung an die Datenschutzerklaerung koppeln oder zur Bedingung der Anfrage machen (Kopplungsverbot Art. 7 IV DSGVO, § 7 UWG, EuGH *Planet49*). Eine so eingesammelte Liste waere unwirksam, rueckwirkend und vollstaendig.
   **Der Anbieter sieht den Verteiler nie.** Toolfolio versendet, nicht er.
...
4. **Sicherheit.** Niemals vollstaendige Kartennummern, PruefZiffern oder IBAN speichern, nur Referenzen (z. B. letzte vier Ziffern). Keine Passwortfelder fuer fremde Tools, um sich dort einzuloggen. Hinterlegte API-Zugangsdaten verschluesselt ablegen, nie ausgeben, nur serverseitig verwenden.
   **Eng begrenzte Ausnahme (freigegeben 2026-06-30, B-24):** Anbieter, deren API ausschliesslich **Basic Auth mit Login + Passwort** kennt (z. B. DataForSEO), duerfen als **API-Credential** hinterlegt werden. Bedingungen, alle zwingend: verschluesselt at rest (AES-256-GCM, Schluessel nur in der Server-Env), Ablage in einer eigenen Tabelle **ohne jede Lese-Policy** (nur Service-Role), niemals an den Client ausgeliefert, niemals geloggt, ausschliesslich **lesende** Endpunkte, jederzeit trennbar/rotierbar. Kein interaktives Login im fremden Tool, keine Weitergabe an die KI-/Aggregat-Ebene.
