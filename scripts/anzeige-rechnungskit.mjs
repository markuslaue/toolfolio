 ...
 *   1. RECHNUNGSKIT als PREMIUM-ANZEIGE bei E-Rechnung Software, oberste Anzeigenposition.
...
/* ------------------------------------------------- 1) RECHNUNGSKIT als Anzeige */

console.log("1) RECHNUNGSKIT auslesen ...");

const urls = ["https://rechnungskit.de/", "https://rechnungskit.de/preise", "https://rechnungskit.de/funktionen"];
const seiten = [];
for (const u of urls) {
...
const PROMPT = `Erstelle das Datenblatt für die Software auf diesen Seiten. Kategorie: E-Rechnung Software.
...
const { data: coll } = await sb.from("dir_collection").select("id, name").eq("slug", "e-rechnung-software").single();
  ...
  .upsert(
    {
      ...
      slug: "rechnungskit",
      anbieter: d.anbieter,
      website_url: "https://rechnungskit.de/",
      farbe: "#2563EB",
...
console.log(`   RECHNUNGSKIT ist Anzeige Nr. 1 bei ${coll.name}.`);


console.log(`\nFertig. Anzeige pruefen: /admin/verzeichnis/collection/e-rechnung-software`);
