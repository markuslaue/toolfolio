...
import { Eye, EyeOff, Loader2, AlertCircle, Check, Gift, CreditCard, Clock } from "lucide-react";
...
export function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setErr(null);
    if (name.trim().length < 2) return setErr("Bitte gib deinen Namen ein.");
    if (!validEmail) return setErr("Bitte gib eine gültige geschäftliche E-Mail ein.");
    if (score < 2) return setErr("Dein Passwort ist zu schwach. Nutze mindestens 8 Zeichen mit Buchstaben und Zahlen.");
    if (!accept) return setErr("Bitte stimme den AGB und der Datenschutzerklärung zu.");

    setLoading(true);
    await new Promise((r) => setTimeout(r, 900));
    setLoading(false);
    toast.success("Konto erstellt. Willkommen bei Toolfolio.");
    navigate({ to: "/onboarding" });
  }

  function handleSSO(name: string) {
    toast.info(`${name}-Registrierung ist in dieser Vorschau nur visuell.`);
  }

  return (
    <AuthShell>
      <div className="rounded-3xl border bg-card p-7 shadow-soft sm:p-8">
        <h1 className="font-display text-3xl font-semibold tracking-tight">Kostenlos starten</h1>
        ...
        <div className="mt-5 rounded-2xl border border-primary/20 bg-primary/8 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm">
              <div className="font-semibold text-foreground">14 Tage voller Agentur-Zugang</div>
              <p className="mt-0.5 text-muted-foreground">
                Danach wählst du deinen Plan oder bleibst auf Free.
              </p>
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                <span className="inline-flex items-center gap-1"><CreditCard className="size-3" /> Keine Kreditkarte</span>
                <span className="inline-flex items-center gap-1"><Clock className="size-3" /> Jederzeit kündbar</span>
                <span className="inline-flex items-center gap-1"><Check className="size-3" /> Alle Funktionen</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <p className="mt-1.5 text-[11px] text-muted-foreground">
              Tipp: mindestens 8 Zeichen, Gross- und Kleinschreibung, Zahl oder Sonderzeichen.
            </p>
          </div>
          ...
          <label className="flex cursor-pointer items-start gap-2.5 rounded-xl border border-border/60 bg-muted/30 p-3 text-sm">
            ...
            <span className="text-foreground/90">
              Ich stimme den{" "}
              <Link to="/" className="text-primary underline hover:no-underline">AGB</Link> und der{" "}
              <Link to="/" className="text-primary underline hover:no-underline">Datenschutzerklärung</Link>{" "}
              zu.
            </span>
          </label>
          ...
          <p className="text-center text-[11px] text-muted-foreground">
            Mit dem Klick auf "Konto erstellen" startest du deinen 14-tägigen Trial. Keine Kreditkarte nötig.
          </p>
        </form>
