...
export async function createService(_prev: AiResult, formData: FormData): Promise<AiResult> {
  ...
  const { data, error } = await supabase
    .from("ai_services")
    .insert({ user_id: user.id, name: parsed.data.name, farbe, budget_monat: budget && budget >= 0 ? budget : null })
    .select("id")
    .single();
...
export async function setBudget(serviceId: string, budget: number | null): Promise<AiResult> {
  ...
  const { error } = await supabase.from("ai_services").update({ budget_monat: wert }).eq("id", serviceId).eq("user_id", user.id);
...
export async function deleteService(serviceId: string): Promise<AiResult> {
  const { supabase, user } = await userClient();
  if (!user) return { error: "Bitte melde dich erneut an." };
  const { error } = await supabase.from("ai_services").delete().eq("id", serviceId).eq("user_id", user.id);
...
export async function upsertSpend(serviceId: string, jahr: number, monat: number, betrag: number): Promise<AiResult> {
  ...
  const { error } = await supabase
    .from("ai_spend")
    .upsert({ user_id: user.id, service_id: serviceId, jahr, monat, betrag }, { onConflict: "service_id,jahr,monat" });
