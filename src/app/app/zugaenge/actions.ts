...
export async function addPerson(_prev: ZugangResult, formData: FormData): Promise<ZugangResult> {
  ...
  const { data, error } = await supabase
    .from("personen")
    .insert({ user_id: user.id, name: parsed.data.name, rolle: parsed.data.rolle || null, email: parsed.data.email || null })
    .select("id")
    .single();
...
export async function removePerson(id: string): Promise<ZugangResult> {
  const { supabase, user } = await uc();
  if (!user) return { error: "Bitte melde dich erneut an." };
  const { error } = await supabase.from("personen").delete().eq("id", id).eq("user_id", user.id);
...
export async function setPersonStatus(id: string, status: "aktiv" | "scheidet_aus" | "ausgeschieden", austritt?: string | null): Promise<ZugangResult> {
  ...
  const { supabase, user } = await uc();
  if (!user) return { error: "Bitte melde dich erneut an." };
  const { error } = await supabase.from("personen").update({ status, austritt: austritt ?? null }).eq("id", id).eq("user_id", user.id);
...
export async function assignTool(personId: string, aboId: string, platzKosten: number | null): Promise<ZugangResult> {
  ...
  const { error } = await supabase
    .from("tool_zugang")
    .upsert({ user_id: user.id, person_id: personId, abo_id: aboId, platz_kosten: platzKosten }, { onConflict: "person_id,abo_id" });
...
export async function unassignTool(zugangId: string): Promise<ZugangResult> {
  const { supabase, user } = await uc();
  if (!user) return { error: "Bitte melde dich erneut an." };
  const { error } = await supabase.from("tool_zugang").delete().eq("id", zugangId).eq("user_id", user.id);
...
export async function setOwner(aboId: string, zugangId: string | null): Promise<ZugangResult> {
  ...
  await supabase.from("tool_zugang").update({ ist_owner: false }).eq("user_id", user.id).eq("abo_id", aboId);
  if (zugangId) {
    const { error } = await supabase.from("tool_zugang").update({ ist_owner: true }).eq("id", zugangId).eq("user_id", user.id);
